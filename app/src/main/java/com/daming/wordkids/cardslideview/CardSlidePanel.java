...
public class CardSlidePanel extends ViewGroup {
    ...
    private List<Card> dataList; // 存储的数据链表
            ...
            public void onClick(View view) {
                if (view.getId() == R.id.maskView) {
                    ...
                    if (null != cardSwitchListener && view.getScaleX() > 1 - SCALE_STEP) {
                        cardSwitchListener.onItemClick(dataList.get(isShowing));
                    }
                }
            }
        };
    ...
    private void orderViewStack() {
        ...
        if (newIndex < dataList.size()) {
            Card dataItem = dataList.get(newIndex);
        ...
        if (isShowing + 1 < dataList.size()) {
            isShowing++;
            if (null != cardSwitchListener) {
                cardSwitchListener.onShow(dataList.get(isShowing));
            }
        }
    }
    ...
    private void animToSide(CardItemView changedView, int xvel, int yvel) {
        ...
        if (finalX == initCenterViewX) {
            ...
            if (flyType >= 0 && cardSwitchListener != null) {
                cardSwitchListener.onCardVanish(dataList.get(isShowing), flyType);
            }
        }
    }
    ...
    private void vanishOnBtnClick(int type) {
        ...
        if (type >= 0 && cardSwitchListener != null) {
            cardSwitchListener.onCardVanish(dataList.get(isShowing), type);
        }
     ...
     */
    public void fillData(List<Card> list) {
        this.dataList = list;
        ...
        isShowing = 0;
        if (null != cardSwitchListener && dataList != null && dataList.size() > 0) {
            cardSwitchListener.onShow(dataList.get(isShowing));
        }
    }
